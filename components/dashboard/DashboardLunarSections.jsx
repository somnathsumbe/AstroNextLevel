...
export default function DashboardLunarSections({ lunar, currentMonthKey, nextMonthKey }) {
    ...
    return <section className="dashboard-lunar-section" aria-labelledby={titleId}>
      ...
      <div className="dashboard-lunar-grid row g-2">
        {dates.map((dateKey) => {
          ...
          return <div key={`${title}-${dateKey}`} className={`dashboard-lunar-card col-12 col-sm-6 col-md-4 col-lg-1 ${isSelected ? 'is-selected' : ''}`} aria-pressed={isSelected}>
            <button type="button" className="dashboard-lunar-date-button" onClick={() => onSelect(dateKey)} aria-label={`Select ${dateKey}`}><span className="dashboard-lunar-date">{dateKey}</span></button>
  ...
  return <div className="dashboard-lunar-wrap">
    {renderLunarSection({ title: 'Amavasya', dates: lunar.amavasya.recent, selectedDate: selectedAmavasyaDate, onSelect: setSelectedAmavasyaDate })}
    {renderLunarSection({ title: 'Purnima', dates: lunar.purnima.recent, selectedDate: selectedPurnimaDate, onSelect: setSelectedPurnimaDate })}
  </div>;
}