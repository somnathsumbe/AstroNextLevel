...
export default function AstroEventSummary({ events }) {
  ...
  return <section className="astro-summary-section" aria-label="Astro event summary"><div className="row g-3">{cards.map(([label, value, icon, tone]) => <div className="col-6 col-md-3" key={label}><StatCard className={`astro-summary-card summary-${tone}`} icon={icon} label={label} value={value} caption={label === 'EVENTS TODAY' ? 'Active calendar events' : value ? 'Active today' : 'No event today'} tone={tone === 'degree' ? 'blue' : tone === 'purnima' ? 'gold' : ''} /></div>)}</div></section>;
}
