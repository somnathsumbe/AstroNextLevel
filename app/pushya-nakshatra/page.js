...
export default function PushyaNakshatraPage() {
  ...
  function shiftMonth(direction) {
    setMonth((current) => shiftMonthOption(current || defaultMonthOption(), direction, PUSHYA_MONTHS));
    setPage(1);
  }
  ...
  return (
    <div className="pushya-page">
      ...
      <form className="filter-panel" onSubmit={applySearch}><div className="filter-title"><span><i className="bi bi-sliders2" /> Calendar Filters</span><span className="filter-count">{filteredRecords.length} records</span></div><div className="row g-3 align-items-end"><div className="col-12 col-md-3"><label htmlFor="pushya-year">Year</label><select id="pushya-year" value={year} onChange={(event) => changeYear(event.target.value)}>{years.map((item) => <option key={item} value={item}>{item}</option>)}</select></div><div className="col-12 col-md-3"><label htmlFor="pushya-month">Month</label><MonthNavigator id="pushya-month" value={month} options={PUSHYA_MONTHS} onChange={changeMonth} onPrev={() => shiftMonth(-1)} onNext={() => shiftMonth(1)} /></div><div className="col-12 col-md-4"><label htmlFor="pushya-search">Search</label><div className="filter-search"><i className="bi bi-search" /><input id="pushya-search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Year, month, start, end or day" /></div></div><div className="col-12 col-md-2"><button className="gold-action w-100" type="submit"><i className="bi bi-search" /> Search</button></div></div></form>
