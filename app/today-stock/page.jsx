...
function getRecentLunarDates() {
  ...
  const amavasya = (amavasyaData.years || []).flatMap((yearData) => (yearData.entries || []).map((entry) => ({
    date: new Date(String(entry.start).replace(' ', 'T')),
    festival: Array.isArray(entry.festival) ? entry.festival.join(', ') : entry.festival,
  }))).filter((entry) => entry.date <= today).sort((a, b) => b.date - a.date).slice(0, 5);
  const purnima = Object.entries(purnimaData.years || {}).flatMap(([year, entries]) => entries.map((entry) => ({
  ...
  return {
    amavasya: amavasya.map((entry) => ({ ...entry, label: formatLunarDate(entry.date) })),
    purnima: purnima.map((entry) => ({ ...entry, label: formatLunarDate(entry.date) })),
  };
}
...
export default function TodayStockPage() {
  ...
  return <main className="container-fluid tool-page today-stock-page"><GannHeader /><GannSummary summary={summary} /><GannFilters values={filters} options={options} icons={icons} onChange={updateFilter} onReset={() => setFilters(initialFilters)} /><section className="lunar-dates-panel" aria-labelledby="lunar-dates-title"><div className="lunar-dates-heading"><div><span className="eyebrow">LUNAR CYCLE / RECENT HISTORY</span><h2 id="lunar-dates-title">Last 5 Amavasya &amp; Purnima</h2></div><button className="outline-action" type="button" onClick={toggleLunarDates} aria-expanded={showLunarDates} aria-controls="lunar-dates-table"><i className={`bi bi-chevron-${showLunarDates ? 'up' : 'down'}`} /> {showLunarDates ? 'Hide Dates' : 'Show Dates'}</button></div>{showLunarDates && <div id="lunar-dates-table" className="lunar-dates-table-wrap"><table className="lunar-dates-table"><tbody><tr><th scope="row"><i className="bi bi-moon" /> Amavasya</th>{lunarDates.amavasya.map((entry) => <td className={copiedDate === entry.label ? 'is-copied' : ''} key={entry.date.toISOString()} title={entry.festival || 'Amavasya'}><span>{entry.label}</span><button type="button" className="lunar-copy-button" onClick={() => copyLunarDate(entry.label)} aria-label={`Copy Amavasya date ${entry.label}`} title={`Copy ${entry.label}`}><i className={`bi bi-${copiedDate === entry.label ? 'check2' : 'clipboard'}`} /></button></td>)}</tr><tr><th scope="row"><i className="bi bi-moon-stars" /> Purnima</th>{lunarDates.purnima.map((entry) => <td className={copiedDate === entry.label ? 'is-copied' : ''} key={`${entry.date.toISOString()}-${entry.name}`} title={entry.name}><span>{entry.label}</span><button type="button" className="lunar-copy-button" onClick={() => copyLunarDate(entry.label)} aria-label={`Copy Purnima date ${entry.label}`} title={`Copy ${entry.label}`}><i className={`bi bi-${copiedDate === entry.label ? 'check2' : 'clipboard'}`} /></button></td>)}</tr></tbody></table></div>}</section>{loading && <div className="state-panel gann-loading"><i className="bi bi-arrow-repeat" /><strong>Loading pressure dates...</strong></div>}{error && <div className="calculator-error" role="alert"><i className="bi bi-exclamation-circle" /> {error}</div>}{!loading && !error && <GannTable rows={filteredData} total={data.length} todayKey={todayKey} todayFilter={filters.today} />}</main>;
}