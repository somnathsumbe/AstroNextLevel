...
export default function BhadraKaalPage() {
  ...
  const filteredRecords = useMemo(() => allRecords.filter((record) => record.year === Number(appliedYear) && record.month === month), [allRecords, appliedYear, month]);
  ...
  function resetFilters() { setSelectedYear(DEFAULT_YEAR); setAppliedYear(DEFAULT_YEAR); setMonth(defaultMonthOption()); setPage(1); }
