...
export default function ShukraGocharPage() {
  ...
  function shiftMonth(direction) {
    setMonth((current) => shiftMonthOption(current || defaultMonthOption(), direction, SHUKRA_MONTHS));
    setPage(1);
  }
  ...
  return (
    <div className="shukra-page">
      ...
      <form className="filter-panel" onSubmit={submitSearch}><div className="filter-title"><span><i className="bi bi-sliders2" /> Shukra Gochar Filters</span><span className="filter-count">{filteredRecords.length} records</span></div><div className="row g-3 align-items-end"><div className="col-12 col-md-3"><label htmlFor="shukra-year">Year</label><select id="shukra-year" value={year} onChange={(event) => changeYear(event.target.value)}>{years.map((item) => <option key={item} value={item}>{item}</option>)}</select></div><div className="col-12 col-md-3"><label htmlFor="shukra-month">Month</label><MonthNavigator id="shukra-month" value={month} options={SHUKRA_MONTHS} onChange={changeMonth} onPrev={() => shiftMonth(-1)} onNext={() => shiftMonth(1)} /></div><div className="col-12 col-md-4"><label htmlFor="shukra-search">Search Shukra Gochar...</label><div className="filter-search"><i className="bi bi-search" /><input id="shukra-search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Date, day, event, rashi or time" /></div></div><div className="col-12 col-md-2"><div className="filter-actions shukra-filter-actions"><button className="gold-action" type="submit"><i className="bi bi-search" /> Search</button><button className="subtle-action" type="button" onClick={clearSearch}>Clear</button></div></div></div></form>
